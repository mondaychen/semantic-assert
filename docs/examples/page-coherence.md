...
title format changes to "Acme Pricing". Each route needs its own copy of the three
strings, and none of the assertions say what actually matters: that the three
places agree with each other.
...
Each claim names one relationship, so a failure says which signal went stale. The
capture is scoped to `main`, which keeps the request small. The document title is
already part of every captured page state, and `extraState` adds the current nav
item under a name the claims can refer to.

::: tip Why pass the nav item explicitly?
The aria snapshot lists the navigation links and their URLs, but it does not
record `aria-current`. Reading the current item with Playwright and handing it to
the judge is more reliable than asking the model to infer it from the URL.
:::
...
renders a three-route app as local HTML with a click-driven router. One test uses a
router that updates everything. A second test uses a router that forgets the title
...
The default provider uses scripted scores. For real judgments, put
...
captured state then includes the real URL, which gives the model one more signal
to compare. Keep exact checks, such as the URL path or an order ID, in ordinary
Playwright assertions. Add a third claim for a breadcrumb or a page-level
`<h1>` outside `main` if your layout keeps them there, and scope the region
accordingly.
